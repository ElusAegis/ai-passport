...
outputs are genuinely produced by those models. By leveraging cryptographic techniques, this system provides a robust
...
In the rapidly evolving landscape of AI, particularly with large language models (LLMs), it is increasingly important to
verify that certain outputs genuinely originated from specific AI models. Users may need to prove that an AI model
produced a harmful or controversial message, and existing centralized solutions (like sharing conversations via AI
service providers) can be insufficient. These centralized platforms can alter or remove content, and providers may have
incentives to hide dangerous behaviors exhibited by their models.

AI Passport addresses this challenge by providing a decentralized, cryptographic method for users to prove that a
particular AI output was generated by a specific model at a specific time. This enhances accountability and
transparency, contributing to AI safety and governance efforts.

### Contributions:

- **Extension of “IDs for AI Systems”**: Building upon the concepts from the
  paper [“IDs for AI Systems”](https://arxiv.org/pdf/2406.12137) by Chan et al., we provide a practical protocol for
  assigning unique identities to AI models and verifying their outputs. This extends theoretical frameworks into
  real-world applications.
- **Integration of Cryptographic Techniques**: By combining Zero-Knowledge Proofs via EZKL for local models and
  TLSNotary for remote models, we offer a comprehensive solution that enhances trust and accountability in AI-generated
  content.
- **Open-Source Implementation**: Our code is openly available, encouraging collaboration and further development in the
  community to advance AI safety and governance.
- **Fix to the TLSNotary**: We identified and resolved an issue in the TLSNotary library that was critical for enabling
  our functionality. Our fix, documented in [Issue #612](https://github.com/tlsnotary/tlsn/issues/612), improves the
  compatibility and performance of TLSNotary for AI output verification.
...
- **Proof Verification**: Verifies the authenticity of the output by checking the proof and ensuring consistency across
  model components.
- **Remote Operations with TLSNotary**: Supports remote interactions with AI services (e.g., Anthropic’s API), allowing
  you to generate and verify proofs of remote AI model executions using TLSNotary.
