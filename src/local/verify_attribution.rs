...
pub async fn verify_attribution(
    ...
    if !model_passport_path.exists() {
        return Err(format!(
            ...
            model_passport_path.display()
        )
        .into());
    }
    if !attribution_certificate_path.exists() {
        return Err(format!(
            "Origin certificate file not found at '{}'.",
    ...
    let proof_json = attribution_certificate_json
        .get("proof")
        .ok_or("Proof not found in the attribution certificate.")
        .map_err(|e| {
            format!(
                "Error extracting proof from the attribution certificate: {}",
                e
            )
        })?;
    let settings_json = attribution_certificate_json
        .get("settings")
        .ok_or("Settings not found in the attribution certificate.")
        .map_err(|e| {
            format!(
                "Error extracting settings from the attribution certificate: {}",
                e
            )
        })?;
    let vk_json = attribution_certificate_json
        .get("vk")
        .ok_or("Verification key not found in the attribution certificate.")
        .map_err(|e| {
            format!(
                "Error extracting VK from the attribution certificate: {}",
                e
            )
        })?;
    ...
    let weights_hash = model_passport_json
        .get("identity_details")
        .ok_or("Identity details not found in the model passport.")
        .map_err(|e| {
            format!(
                "Error extracting identity details from the model passport: {}",
                e
            )
        })?
        .get("weight_hash")
        .ok_or("Weight hash not found in the model passport.")
        .map_err(|e| {
            format!(
                "Error extracting weight hash from the model passport: {}",
                e
            )
        })?
        .as_str()
        .ok_or("Error decoding weight hash")?
        .to_string();

    // Save the proof, vk, and settings to a temporary file
    std::fs::write(&proof_path, serde_json::to_string(proof_json)?)
        .map_err(|e| format!("Error saving proof to file: {}", e))?;
    std::fs::write(&settings_path, serde_json::to_string(settings_json)?)
    ...
    let verified_model_identity_hash = verified_model_identity
        .unique_indentifier()
    ...
    let model_identity_in_passport =
        extract_model_id_from_passport(model_passport_path).map_err(|e| {
            format!(
                "Error extracting model ID from the attribution certificate: {}",
                e
            )
        })?;
...
fn extract_model_id_from_passport(model_passport_path: &Path) -> Result<String, Box<dyn Error>> {
    ...
    let model_id = attribution_certificate_json
        .get("model_identity_hash")
        .ok_or("Model ID key `model_identity_hash` not found in the model passport.")
        .map_err(|e| format!("Error extracting model ID from the model passport: {}", e))?;
    Ok(model_id
        .as_str()
        .ok_or("Error decoding model ID")?
        .to_string())
}
