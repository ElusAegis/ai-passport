...
pub async fn prove_attribution(model_path: &Path, input_json: &Path, save_to_path: Option<&Path>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    generate_witness(&compiled_model_path, input_json, &witness_path).await.map_err(|e| format!("Error generating the witness: {}", e))?;

    // Step 6: Generate the proof
    generate_proof(&compiled_model_path, &pk_path, &witness_path, &proof_path).await.map_err(|e| format!("Error generating the proof: {}", e))?;

    // Generate the attribution certificate (JSON)
    let output_dir = save_to_path.unwrap_or_else(|| Path::new("."));
    let attribution_certificate_path = create_attribution_certificate(model_path, &proof_path, &settings_path, &vk_path, output_dir).map_err(|e| format!("Error creating the attribution certificate: {}", e))?;
...
fn create_attribution_certificate(
    ...
    let proof_json: Value = serde_json::from_str(&proof_data).map_err(|e| format!("Failed to parse proof JSON: {}", e))?;
    ...
    let attribution_certificate_path = output_dir.join(format!("model_{}_attribution_certificate.json", model_identity_hash[0..8].to_string()));
